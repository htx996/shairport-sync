...
```yaml
docker.io/${DOCKERHUB_NAMESPACE:-REPLACE_ME}/shairport-sync-airplay2:latest
docker.io/${DOCKERHUB_NAMESPACE:-REPLACE_ME}/shairport-sync-webui:latest
```

发布到自己的 Docker Hub namespace 后，在 NAS 上设置 namespace 再运行：

```bash
export DOCKERHUB_NAMESPACE=你的DockerHub用户名或组织名
...
```text
docker.io/<namespace>/shairport-sync-airplay2:latest
docker.io/<namespace>/shairport-sync-webui:latest
docker.io/<namespace>/shairport-sync-airplay2:<commit-sha>
docker.io/<namespace>/shairport-sync-webui:<commit-sha>
```
